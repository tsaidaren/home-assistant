...
def mock_august_authentication(token_text, token_timestamp):
...
def test_refresh_access_token():
    ...
    authentication = mock_august_authentication("original_token", 1234)
    authenticator = mock_august_authenticator()
    ...
    authenticator.refresh_access_token.return_value = mock_august_authentication(
        "new_token", 5678
    )
    data.refresh_access_token_if_needed()
